...
from dersler.models import Baslik, Ders, Question
...
def ogretmen_profil(request, ogretmen_id):
	...
	return render(request, 'profil/ogretmen.html', context)
...
def ogrenci_profil(request, ogrenci_id):
	...
	return render(request, 'profil/ogrenci.html', context)
...
def favoriler(request, ogrenci_id):
	...
	return render(request, 'profil/favoriler.html', context)


def derslerim(request, ogrenci_id):
	user = get_object_or_404(User, pk=ogrenci_id)
	dersler = Ders.objects.filter(baslik__owner = user).select_related("baslik").prefetch_related("question_set")
	sahip_mi = user == request.user
	...
	return render(request, 'profil/derslerim.html', context)
...
def sorular(request, ogrenci_id):
	...
	return render(request, 'profil/sorular.html', context)
...
def ders_olustur(request, ogrenci_id):
	...
	if user == ogrenci:
		...
		return render(request, 'profil/ders_olustur_formu.html', {'form': form})
	else:
