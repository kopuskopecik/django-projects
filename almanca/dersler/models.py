...
from hesaplar.models import User
...
class Baslik(models.Model):
	...
	def degistir(self):
		return reverse('dersler:baslik_guncelle', kwargs={'pk':self.pk})
		
	def get_absolute_url(self):
		return reverse('dersler:baslik_detail', kwargs={'pk':self.pk})
...
class Ders(models.Model):
	...
	def guncelle(self):
		return reverse('dersler:ders_guncelle', kwargs={'baslik_pk':self.baslik.pk, 'ders_pk':self.pk})
	
	def sil(self):
		return reverse('dersler:ders_sil', kwargs={'pk':self.pk})
...
class Question(models.Model):
	...
	def sil(self):
		return reverse('dersler:soru_sil', kwargs={'pk':self.pk})
	
	def cevap_sayisi(self):
		return self.answer_set.all().count()
	
	def get_absolute_url(self):
		return reverse('dersler:sorulara_cevap_ekle', kwargs={'baslik_pk':self.ders.baslik.pk, 'ders_pk':self.ders.pk, 'soru_pk':self.pk})
...
class TakenQuiz(models.Model):
	student = models.ForeignKey('hesaplar.Student', on_delete=models.CASCADE)
