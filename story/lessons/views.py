...
def baslik_detail(request, pk):
	...
	#lessons = baslik.ders_set.all()
	lessons = baslik.ders_set.all().prefetch_related(
	...
	return render(request, 'lessons/baslik_detail.html', context)
	
@method_decorator([login_required], name='dispatch')
class BaslikOlusturView(CreateView):
    model = Baslik
    #fields = ('title', 'content', 'ranking', 'youtube_urli',)
    template_name = 'lessons/baslik_ekle.html'
    ...
    def form_valid(self, form):
        ...
        return redirect('profiles:ogrenci_profil', user.pk)
...
class BaslikUpdateView(UpdateView):
    ...
    template_name = 'lessons/baslik_guncelleme_formu.html'
	
    def get_context_data(self, **kwargs):
        lessons = self.get_object().ders_set.all()
    ...
    def get_success_url(self):
        return reverse('lessons:baslik_guncelle', kwargs={'pk': self.object.pk})
		
@method_decorator([login_required], name='dispatch')
class BaslikDeleteView(DeleteView):
    model = Baslik
    context_object_name = 'baslik'
    template_name = 'lessons/baslik_silme_onaylama.html'
    success_url = reverse_lazy('lessons:baslik_listesi')
...
def ders_ekle(request, baslik_pk):
    ...
    return render(request, 'lessons/ders_ekle_formu.html', {'baslik': baslik, 'form': form})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            with transaction.atomic():
                form.save()
                formset.save()
            messages.success(request, 'Baslik ve lessons başarıyla oluşturuldu!')
            return redirect('lessons:baslik_guncelle', baslik.pk)
    else:
        form = BaslikOlusturmaFormu(instance=baslik)
        formset = DersFormSet(instance=baslik)

    return render(request, 'lessons/baslik_degistirme_formu.html', {
        'baslik': baslik,
		'ders': ders,
        'form': form,
        'formset': formset
...
def ders_guncelle(request, baslik_pk, ders_pk):
	...
	return render(request, 'lessons/ders_guncelleme_formu.html',context)
...
def soru_ekle(request, baslik_pk, ders_pk):
    ...
    return render(request, 'lessons/soru_ekle_formu.html', {'baslik': baslik, 'form': form, 'ders':ders})
...
def sorulara_cevap_ekle(request, baslik_pk, ders_pk, soru_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            return redirect('lessons:ders_guncelle', baslik.pk, ders.pk)
    else:
        form = QuestionForm(instance=soru)
        formset = AnswerFormSet(instance=soru)

    return render(request, 'lessons/sorulara_cevap_ekleme_formu.html', {
        'baslik': baslik,
		'ders': ders,
        'form': form,
        'formset': formset,
...
def take_quiz(request, pk):
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                if student.get_unanswered_questions(quiz).exists():
                    return redirect('lessons:take_quiz', pk)
                else:
    ...
    return render(request, 'lessons/take_quiz_form.html', {
        'quiz': quiz,
        'question': question,
        'form': form,
        'progress': progress
...
def take_baslik_quiz(request, pk):
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                if student.get_unanswered_baslik_questions(baslik, questions).exists():
                    return redirect('lessons:take_baslik_quiz', pk)
                else:
    ...
    return render(request, 'lessons/take_baslik_quiz_form.html', {
        'baslik': baslik,
        'question': question,
        'form': form,
        'progress': progress,
...
def alistirma_yap(request, pk):
	...
	lessons = baslik.ders_set.all().prefetch_related("question_set__answer_set")
	sorular = []
	for ders in lessons:
	...
	return render(request, 'lessons/alistirma_yap.html', context)
